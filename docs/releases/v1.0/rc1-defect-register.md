...
| RC1-P0-004 | P0 BLOCKER | Rollback | Confirmed blocker | The historical `c1b6958` artifact is preserved evidence, but it is not an executable RC rollback target because reproducible runtime image construction fails with `ERR_PNPM_NO_LOCKFILE`. Legacy `staging-api:latest` and `staging-web:latest` images are also rejected because they lack verifiable artifact SHA, source commit, release version, deployment ID, and provenance labels. | `docs/releases/v1.0/rc1-rollback-rehearsal.md`, `docs/releases/v1.0/rc1-redeploy-validation.md`, `docs/releases/v1.0/rc1-rollback-baseline.md`, and `docs/releases/v1.0/rc1-forward-rehearsal-candidate.md` document the finding and the prepared forward candidate. The forward artifact is `laborator-editura-1.0.0-rc.1-rehearsal.1-add6e73.tar.gz`, SHA-256 `05ec1fb248aceb8b88efd66b6309a6ba928e24152ad83997fd549c5da26d66a4`, source commit `add6e73221d70fbc07d0f724a8322d5aa3b503d9`, migration `0008_security_hardening_phase_1.sql`. Runtime images were not built locally because Docker is unavailable. | Build verified forward runtime images from the artifact on the VPS, deploy them with `--no-build`, rehearse rollback to verified `30b39ec`, validate health/data integrity, and redeploy through the canonical artifact path. Do not use `c1b6958`, `--no-frozen-lockfile`, or legacy images. | OPEN |
...
reproducible and the prepared forward candidate has not been deployed live,
rolled back to the verified `30b39ec` baseline, and redeployed through the
canonical artifact path. RC1 still cannot proceed to pilot with the remaining
open P0 blocker.
